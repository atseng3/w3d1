...
SELECT * FROM
(SELECT DISTINCT stopa.name AS cstop, a.company, a.num
FROM route a JOIN route b ON
(a.company = b.company AND a.num = b.num)
JOIN stops stopa ON (a.stop = stopa.id)
JOIN stops stopb ON (b.stop = stopb.id)
WHERE stopb.name = 'Craiglockhart') AS craig_stops
JOIN (SELECT DISTINCT stopa.name AS sstop, a.company, a.num
FROM route a JOIN route b ON
(a.company = b.company AND a.num = b.num)
JOIN stops stopa ON (a.stop = stopa.id)
JOIN stops stopb ON (b.stop = stopb.id)
WHERE stopb.name = 'Sighthill') AS sight_stops
ON sight_stops.sstop = craig_stops.cstop